...
curl -i $(cat target/outputs.json | jq -r '.LambdaCustomRuntimeRICInfrastructureStack.apiendpoint')/custom-runtime-x86
curl -i $(cat target/outputs.json | jq -r '.LambdaCustomRuntimeRICInfrastructureStack.apiendpoint')/custom-runtime-arm
